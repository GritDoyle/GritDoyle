...
def recruit(page):
    ...
    session = HTMLSession()
    r = session.get(
        'https://www.xmrc.com.cn/net/info/Resultg.aspx?a=a&g=g&recordtype=1&searchtype=3&keyword=运维&releasetime=365&worklengthflag=0&jobtype=9902&sortby=updatetime&ascdesc=Desc&pagesize=20&PageIndex=%d'%page
    )
...
def save_recruit_info():
    # 新建一个csv的文件
    recruit_info = open('爬取厦门人才网/recruit_info_om.csv', 'w', encoding='gbk', newline='')
    ...
    recruit_info.close()
    # 创建MySQL数据库连接
    data = pd.read_csv(open("爬取厦门人才网/recruit_info_om.csv", encoding="gbk"), sep=',', header=0)
    print(data)
    engine = create_engine("mysql+pymysql://root:@localhost:3306/studyfiles?charset=utf8")
    data.to_sql('厦门人才网_om',engine,index=True)
    print('成功存入数据库!')
