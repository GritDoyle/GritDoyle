...
def douban():
    ...
    items = soup.find("ul", id="listCont1").find_all('li',class_='clearfix')
    movies=[]
    # 遍历所有列表项
    for item in items:
        movie={} #临时存储
        # 电影名
        try:
            movie['电影名'] = item.find('div', class_='box_chart').get_text()
        except:
            movie['电影名'] = ""
        # 波动排名
        try:
            movie['票房'] = item.find('span', class_='box_chart_num color-gray').get_text()
        except:
            movie['票房'] = ''
        # 现排名
        try:
...
def save_douban():
    # 新建一个csv文件
    movie=open('北美票房榜（12月3日更新）.csv','w',encoding='utf-8',newline='')
    writer = csv.writer(movie)  # CSV写入的参数为一个list
    writer.writerow(['电影名', '票房', '现排名'])
    page=1
    while page==1:
